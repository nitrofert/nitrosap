...
@Component({
  selector: 'app-lista-precios-item-sugerido',
  providers: [MessageService, ConfirmationService, DialogService],
  templateUrl: './lista-precios-item-sugerido.component.html',
  styleUrls: ['./lista-precios-item-sugerido.component.scss']
})
export class ListaPreciosItemSugeridoComponent  implements OnInit {

  @ViewChild('filter') filter!: ElementRef;

  listaPreciosSugeridos:any[] = [];
  selectedLine:any[] = [];
  ...
  getListaPreciosSugeridos(){
    this.loading = true;
    this.comprasService.getListaPreciosSugeridos(this.authService.getToken())
        .subscribe({
           next:(items)=>{
              console.log(items);
              this.listaPreciosSugeridos= items;
              this.loading = false;
              },
           error:(err)=>{
              console.error(err);
...
adicionarCSV(){
 ...
 //if(this.lineasPrecioMercado.length>0){
    ...
    body.append('myFileLPSugerido', this.fileTmp2.fileRaw, this.fileTmp2.fileName); 

    console.log(body);

        this.comprasService.cargarLPSugerido(this.authService.getToken(),body)
        .subscribe({
          next: (result:any)=>{
            console.log(result);
...
descargarCSV(){}
  ...
  exportExcel() {
    import("xlsx").then(xlsx => {
        const worksheet = xlsx.utils.json_to_sheet(this.listaPreciosSugeridos);
